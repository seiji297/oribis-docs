...
fix・feature において GUI 上で変化が確認可能な修正を行った場合、**テスト項目に GUI 確認を必ず含めること**。

- テスト計画に「GUI確認項目」セクションを設け、何を・どう確認するかを明記する
- GUI上の確認は自動テストでは代替不可。**Producerに依頼して目視確認を受けること**
- GUI確認が不要な変更（バックエンド内部ロジックのみ等）はその旨を明記する
...
- [ ] GUI上で確認可能な変更がある場合、GUI確認項目をテスト計画に含めたか
- [ ] GUI確認をProducerに依頼したか（自動テストのみで完了扱いにしていないか）
